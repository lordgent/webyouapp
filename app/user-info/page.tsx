...
export default function UserInfo() {
  ...
  const [about, setAbout] = useState<UserProfile | any>({
    birthday: "", age: 0, horoscope: "--", zodiac: "--",
    height: 0, weight: 0, displayName: "", gender: "", image: "", interests: []
  });
  ...
  const fetchProfile = async () => {
    try {
      ...
      const mapped = { ...data, displayName: data.name };
      setAbout(mapped);
      setTempAbout(mapped);
  ...
  const handleSave = async (type: 'about' | 'interest') => {
    ...
    try {
      ...
      const finalData = { ...updated, displayName: updated.name };
  ...
  return (
    <BaseLayout>
      ...
      <div className="flex items-center p-4">
        ...
        <p className="flex-1 text-center font-semibold text-sm">@{about.displayName}</p>
      ...
      <div className="max-w-md mx-auto px-4 space-y-6">
        ...
        <div className="relative w-full aspect-[4/3] bg-[#162329] rounded-2xl overflow-hidden flex flex-col justify-end p-5 shadow-xl">
          {about.image && <img src={about.image} className="absolute inset-0 w-full h-full object-cover opacity-60" />}
          <div className="relative z-10">
            <p className="font-bold text-lg">@{about.displayName}{about.age ? `, ${about.age}` : ''}</p>
        ...
        <div className="bg-[#0E191F] rounded-2xl p-5">
          ...
          {!editAbout ? (
            <p className="text-sm text-gray-500">Add in your your to help others know you better</p>
          ) : (
            <div className="space-y-4">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div onClick={() => fileInputRef.current?.click()} className="w-14 h-14 bg-[#1F2C33] rounded-2xl flex items-center justify-center cursor-pointer border border-dashed border-[#D4B785]/20 overflow-hidden">
                  {tempAbout.image ? <img src={tempAbout.image} className="w-full h-full object-cover" /> : <Plus size={24} className="text-[#D4B785]" />}
                </div>
                <span className="text-xs">Add image</span>
              </div>
              <EditRow label="Display name:"><input className="custom-input" placeholder="Enter name" value={tempAbout.displayName} onChange={e => setTempAbout({ ...tempAbout, displayName: e.target.value })} /></EditRow>
              ...
              <EditRow label="Birthday:"><input type="text" className="custom-input" placeholder="DD MM YYYY" value={tempAbout.birthday} onChange={e => setTempAbout({ ...tempAbout, birthday: e.target.value })} /></EditRow>
              <EditRow label="Height:"><input type="number" className="custom-input" placeholder="Add height" value={tempAbout.height || ""} onChange={e => setTempAbout({ ...tempAbout, height: e.target.value })} /></EditRow>
              <EditRow label="Weight:"><input type="number" className="custom-input" placeholder="Add weight" value={tempAbout.weight || ""} onChange={e => setTempAbout({ ...tempAbout, weight: e.target.value })} /></EditRow>
            </div>
          )}
        </div>
        ...
        <div className="bg-[#0E191F] rounded-2xl p-5">
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-bold text-sm">Interest</h2>
            {editInterest ? (
          ...
          {!editInterest ? (
            <div className="flex flex-wrap gap-2">
              {about.interests?.length > 0 ? (
                about.interests.map((item: string, idx: number) => (
                  <span key={idx} className="bg-white/5 px-4 py-2 rounded-full text-xs font-medium">{item}</span>
                ))
              ) : (
            ...
            <div className="animate-in fade-in slide-in-from-top-1">
              <input
                ...
                placeholder="Music, Sport, Tech (separated by comma)"
